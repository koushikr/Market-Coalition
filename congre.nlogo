...
;preferences- item x preferences = ordering of item number x in the preference list, 0 represents the most preferred item
;valuations- item x valuations   = value of item number x. 
;payment- valuation of the good received
; we start with a reservation-price of 10 for the agents.. 
...
; just one execution of update means a market start to market close.. keep on looping forever, that would mean, you would know the shifting of congregations.. 
; profit is the average net profit w.r.t to an agent
; comp is the computation cost every agent has to spend for the computations.. every message exchange is 0.1
...
to update
  ...
  set profit (sum [payment] of sellers ) / num-sellers ; total payment of all the sellers upon num-sellers will give the average netprofit of each seller.. 
  set comp-cost (sum [computational-cost] of markets ) / num-markets 
  ;total computational cost of markets upon number of markets will give the computational cost that each agent has to share
  
  ; Below are the plots - Profit represents average profit for the system. 
...
to do-auction
  ...
  ;they receive payment of 0 if they are unable to do a transaction
  ask producers [set payment 0]
  ask consumers [set payment 0]

  ;cost is calculated like this. 
...
;    set next-producer min-one-of producers [min values-from consumers [item (good-of myself) preferences]]
    ...
    ;lower the rank, higher the preference.. 
...
to setup-preferences
  ...
  set valuations []
  while [not empty? op][
    set valuations lput (reservation-price * (1 - (first op / num-goods ))) valuations
    set op butfirst op]
end

; go-to-market helps an agent pick up a market at random depending on the payment they get at the close of market.. 
to go-to-market

  let nm 0
  
  ; tau is the threshold.. and tau times reservation price is what an agent checks for while deciding to stay in the congregation or not...   
  if (payment < tau * reservation-price)[

    set nm one-of markets with [who != market]

    ;If there's no other market, then stop.. this error checking condition may not be necessary -- but on a safer note, this one's put up.. 
    if (nm = nobody) [stop]  
